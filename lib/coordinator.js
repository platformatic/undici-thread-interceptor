...
function addRoute (interceptor, url, ports, meta) {
  ...
  for (const port of ports) {
    ...
    for (const [key, roundRobin] of routes) {
      const first = key.replace(interceptor[kDomain], '').toLowerCase()
      const second = url.replace(interceptor[kDomain], '').toLowerCase()

      if (first === second || interceptor[kHooks].fireOnChannelCreation(first, second) === false) {
        continue
      }
