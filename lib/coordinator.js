...
async function wire (interceptor, port, url, ready, address) {
  ...
  for (const [key, roundRobin] of routes) {
    ...
    for (const other of roundRobin) {
      ...
      if (wired) {
        promises.push(waitMessage(other, { timeout: 5000, description: 'MESSAGE_ROUTE_UPDATED' }, m =>
      ...
      promises.push(waitMessage(port, { timeout: 5000, description: 'MESSAGE_ROUTE_ADDED' }, m =>
        m.type === MESSAGE_ROUTE_ADDED &&
        m.threadId === other[kThread]
      ))
      promises.push(waitMessage(other, { timeout: 5000, description: 'MESSAGE_ROUTE_ADDED' }, m =>
        m.type === MESSAGE_ROUTE_ADDED &&
        m.threadId === port[kThread]
      ))
...
async function removeThreadRoutes (interceptor, threadId) {
  ...
  for (const otherPort of otherPorts) {
    promises.push(waitMessage(otherPort, { timeout: 5000, description: 'MESSAGE_ROUTE_REMOVED' }, m =>
...
async function close (interceptor) {
  ...
  for (const [url, port] of ports) {
    ...
    const closedPromise = waitMessage(
      port,
      { timeout: 5000, description: 'MESSAGE_CLOSE' },
