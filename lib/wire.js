...
async function close (interceptor, port) {
  ...
  // Wait for the queue to drain - this ensures all messages received
  // (including from mesh connections) have been added to kInflightIncoming
  await interceptor[kQueue].drained()
...
function replaceServer (interceptor, port, server) {
  ...
  port.postMessage({ type: MESSAGE_WIRE, ready, address, threadId })
}
