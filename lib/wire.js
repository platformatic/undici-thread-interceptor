...
function onWireMessage (interceptor, port, message) {
  ...
  switch (message.type) {
    case MESSAGE_REQUEST:
      ...
      if (currentLoop === lastLoop || !interceptor[kQueue].idle()) {
...
function createWire (interceptor, newServer, port) {
  ...
  interceptor[kQueue] = fastq.promise(onRequest.bind(null, interceptor), MAX_QUEUE)
  interceptor[kQueue].drain = function () {
    lastLoop = uvMetricsInfo.loopCount
  }
  interceptor[kQueue].saturated = function () {
    interceptor[kQueue].pause()
    setImmediate(function () {
      lastLoop = uvMetricsInfo.loopCount
      interceptor[kQueue].resume()
    })
  }
  interceptor[kHasInject] = false
