...
function createWire (interceptor, newServer, port) {
  ...
  function resetloop (cb) {
    lastLoop = uvMetricsInfo.loopCount
    cb()
  }
  ...
  // for network events.
  interceptor[kQueue] = fastq(function (msg, cb) {
    onRequest(interceptor, msg)
    const currentLoop = uvMetricsInfo.loopCount
    const isSameLoop = currentLoop === lastLoop
    if (isSameLoop && pipelining >= MAX_QUEUE) {
      setImmediate(resetloop, cb)
    } else {
      lastLoop = uvMetricsInfo.loopCount
      if (isSameLoop) {
        pipelining++
      ...
      // Most of the time the callback is called synchronously
      // so that we can process the next request without
      // unnecessary delays.
      cb()
    }
  }, MAX_QUEUE)
