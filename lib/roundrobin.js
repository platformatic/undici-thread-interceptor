...
const { kMeta, kThread, kReady } = require('./utils')
  ...
  findAccepting (canAcceptFn, context) {
    ...
    for (let i = 0; i < this.ports.length; i++) {
      ...
      if (canAcceptFn({ ...context, port, meta: port[kMeta] })) {
        // Update index to maintain round-robin behavior
        this.index = (portIndex + 1) % this.ports.length
        return port
      }
    }
    return null // All workers rejected
  }
